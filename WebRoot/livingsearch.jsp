  ...
  <div class="headBox"> 
    <div class="searchBox" >
      ...
      <div class="addressBox">
        ...
        <div class="put"><input type="text" name="theme" placeholder="目的乡村" id="villageIpt"  ></div>
      </div>
      <div class="IntimeBox">
            ...
            <div class="txtBox">
             <%for(Room ml:minlist){
                if(ml.getHotelName().equals(hl.getHotelName())) {
                  if(ml.getPrice()!=null){
             %>
              <p class="row1">￥<span class="Hprice"><%=ml.getPrice() %></span>起<a href="${pageContext.request.contextPath}/livingdetail.do?hotelName=<%=hl.getHotelName() %>"><span class="Hname"><%=hl.getHotelName() %></span></a></p>
...
<!-- 分页功能 start -->  
    <div align="center">  
        <font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第  
            ${page.pageNow} 页</font> <a href="${pageContext.request.contextPath}/livingsearch.do?pageNow=1&theme=<%=request.getAttribute("theme") %>&inDate=<%=request.getAttribute("inDate") %>&outDate=<%=request.getAttribute("outDate") %>&hotelName=<%=request.getAttribute("hotelName") %>">首页</a>  
        <c:choose>  
            <c:when test="${page.pageNow - 1 > 0}">  
                <a href="${pageContext.request.contextPath}/livingsearch.do?pageNow=${page.pageNow - 1}&theme=<%=request.getAttribute("theme") %>&inDate=<%=request.getAttribute("inDate") %>&outDate=<%=request.getAttribute("outDate") %>&hotelName=<%=request.getAttribute("hotelName") %>">上一页</a>  
            </c:when>  
            <c:when test="${page.pageNow - 1 <= 0}">  
                <a href="${pageContext.request.contextPath}/livingsearch.do?pageNow=1&theme=<%=request.getAttribute("theme") %>&inDate=<%=request.getAttribute("inDate") %>&outDate=<%=request.getAttribute("outDate") %>&hotelName=<%=request.getAttribute("hotelName") %>">上一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="${pageContext.request.contextPath}/livingsearch.do?pageNow=${page.pageNow}&theme=<%=request.getAttribute("theme") %>&inDate=<%=request.getAttribute("inDate") %>&outDate=<%=request.getAttribute("outDate") %>&hotelName=<%=request.getAttribute("hotelName") %>">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 < page.totalPageCount}">  
                <a href="${pageContext.request.contextPath}/livingsearch.do?pageNow=${page.pageNow + 1}&theme=<%=request.getAttribute("theme") %>&inDate=<%=request.getAttribute("inDate") %>&outDate=<%=request.getAttribute("outDate") %>&hotelName=<%=request.getAttribute("hotelName") %>">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">  
                <a href="${pageContext.request.contextPath}/livingsearch.do?pageNow=${page.totalPageCount}&theme=<%=request.getAttribute("theme") %>&inDate=<%=request.getAttribute("inDate") %>&outDate=<%=request.getAttribute("outDate") %>&hotelName=<%=request.getAttribute("hotelName") %>">下一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="${pageContext.request.contextPath}/livingsearch.do?pageNow=${page.pageNow}&theme=<%=request.getAttribute("theme") %>&inDate=<%=request.getAttribute("inDate") %>&outDate=<%=request.getAttribute("outDate") %>&hotelName=<%=request.getAttribute("hotelName") %>">尾页</a>  
            </c:when>  
            <c:otherwise>  
                <a href="${pageContext.request.contextPath}/livingsearch.do?pageNow=${page.totalPageCount}&theme=<%=request.getAttribute("theme") %>&inDate=<%=request.getAttribute("inDate") %>&outDate=<%=request.getAttribute("outDate") %>&hotelName=<%=request.getAttribute("hotelName") %>">尾页</a>  
            </c:otherwise>  
        </c:choose>  
    </div>  
    <!-- 分页功能 End -->  
