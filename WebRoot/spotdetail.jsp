...
<div id="hidebg"></div><!--遮罩层-->
  ...
  <div class="topBox">
    <div class="titleBox">
      <div class="titleName"><%=spot.getSpotName() %></div> 
    </div><!-- titleBox结束 -->
    <div class="addressBox">
     <div class="leftBox">
       <div class="topBox">
         <div class="imgBox"><img src="img/address.png"></div>  
         <div class="txtBox"><a href="#"><%=spot.getAddress() %></a></div>
       </div>
       <div class="bottomBox">
         <div class="businessTime"><%=spot.getService() %></div>
       </div>
     </div> 
      <%for(Ticket ml:minlist){
          if(ml.getSpotName().equals(spot.getSpotName())) {
             if(ml.getPriceofticket()!=null){
       %>
      <div class="middleBox">￥<span><%=ml.getPriceofticket() %></span>起</div>
      <% } 
        }
       }%>
      <div class="rightBox">
    ...
    <div class="secTitleBox"><h3>景点预览</h3></div><!-- secTitleBox结束 -->
    <div class="box">
    <div class="secBox">
      <ul id="loopPics">
      </ul>
  ...
  <div class="homeTypeName">门票列表</div>
  <div class="homeList" id="001">
    <div class="homeListName">
      <div class="List1">门票类型</div>
      <div class="List6">政策</div><!-- 政策 -->
      <div class="List7">价格</div><!-- 房费 -->
    </div> 
      <%List<Ticket> tlist = (List<Ticket>)request.getAttribute("ticketlist"); 
          for(Ticket tl:tlist){
           if(tl.getSpotName().equals(spot.getSpotName())){
              if(tl.getPriceofticket()!=null){%>
      <ul class="masterUl">
        <li class="masterLi">
          <div class="List1">
            <div class="List1Name"><%=tl.getTypeofticket() %></div>
          </div>
          <ul class="subUl">
            <li>
                <div class="List6"><%=tl.getPolicy() %></div><!-- 政策 -->
                <div class="List7">￥<span><%=tl.getPriceofticket() %></span></div><!-- 房费 -->
                <%if(user == null){%>
                  ...
                  <%}else{ %>
                  <a href="${pageContext.request.contextPath}/spotorder.do?orderName=<%=spot.getSpotName() %>&theme=<%=spot.getTheme() %>&address=<%=spot.getAddress() %>&type=<%=tl.getTypeofticket() %>&price=<%=tl.getPriceofticket() %>&wayofpayment=<%=tl.getWayofpayment() %>"><div class="List8"><div class="orderBtn"><div class="BtnTop">预定</div><div class="BtnBottom">在线付</div></div></div></a><!-- 按钮 -->
                <%} %>
            </li>
    ...
    <div class="homeIntroName">景点介绍</div>
    <div class="homeIntroContent"><p><%=spot.getIntroduce() %></p></div>

    <div class="homeOrderName">预定须知</div>
    <div class="homeOrderContent"><%=spot.getIssue() %></div>

    <div class="homeRouteName">路线建议</div>
    <%if(spot.getGobybus()!=null||spot.getGobycar()!=null){ %>
    <div class="homeRouteContent"><%=spot.getGobycar() %></div>
    <div class="homeRouteContent"><%=spot.getGobybus() %></div>
    <%} %>
    ...
    <div class="news_comment_title"><h3>民宿推荐</h3></div>
          ...
          <div class="homeList">
          <%List<Hotel> spotHotels = (List<Hotel>)request.getAttribute("spotHotels");
            List<Room> minroomlist = (List<Room>)request.getAttribute("minpriceroomlist");
            for(Hotel sh:spotHotels){ %>
           <div class="homeItem">
            <div class="homeImg">
              <img src="<%=sh.getImage() %>">
            </div>
            <div class="homeIntro">
              <h4><a href="livingdetail.do?hotelName=<%=sh.getHotelName() %>"><%=sh.getHotelName() %></a></h4>
              <a href="#"><span class="intro1"><%=sh.getAddress() %></span></a>
              <span class="intro2"><%=sh.getPoint() %></span>
            </div>
            <div class="homePrice">
              <%for(Room minl:minroomlist){
                   if(minl.getHotelName().equals(sh.getHotelName())) {
                     if(minl.getPrice()!=null){
               %>
              <span>￥</span><span class="price"><%=minl.getPrice() %></span><span>起</span>
              <% } 
                }
              }%>
            </div>
    ...
    <div class="news_comment">
      ...
      <form action="${pageContext.request.contextPath}/commentsspotsubmit.do?spotName=<%=spot.getSpotName() %>"	method="post">
