...
<div class="transBtn">
  <a onclick="location.href='busmap.jsp?xy='+document.getElementById('gj').title" target="_blank" title="" id="gj" >到这里去</a>
</div>
  ...
  <form style="display: flex;" action="${pageContext.request.contextPath}/spotsearch.do"
    ...
    <div id="demo1" class="slideBox">
      ...
      <ul class="items">
        ...
        <li><a href="${pageContext.request.contextPath}/spotdetail.do?spotName=<%=maxspot.getSpotName() %>" title="<%=maxspot.getSpotName() %>"><img src="<%=maxspot.getImage() %>" style="width:600px;height:300px;"></a></li>
        <%}%>
      </ul>
    ...
    <ul>
      ...
      <div class="baseBox">
        ...
        <div class="txtBox">
          ...
          <div class="addressBox">位置：<span><%=sl.getAddress() %></span><a class="mapBtn" id="1" title="<%=sl.getPos()%>" href="javascript:void(0)">点击在地图中显示</a></div>
        </div>
        <div class="priceBox">
...
<!-- 分页功能 start -->  
    <div align="center">  
        <font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第  
            ${page.pageNow} 页</font> <a href="${pageContext.request.contextPath}/spot.do?pageNow=1">首页</a>  
        <c:choose>  
            <c:when test="${page.pageNow - 1 > 0}">  
                <a href="${pageContext.request.contextPath}/spot.do?pageNow=${page.pageNow - 1}">上一页</a>  
            </c:when>  
            <c:when test="${page.pageNow - 1 <= 0}">  
                <a href="${pageContext.request.contextPath}/spot.do?pageNow=1">上一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="${pageContext.request.contextPath}/spot.do?pageNow=${page.pageNow}">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 < page.totalPageCount}">  
                <a href="${pageContext.request.contextPath}/spot.do?pageNow=${page.pageNow + 1}">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">  
                <a href="${pageContext.request.contextPath}/spot.do?pageNow=${page.totalPageCount}">下一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="${pageContext.request.contextPath}/spot.do?pageNow=${page.pageNow}">尾页</a>  
            </c:when>  
            <c:otherwise>  
                <a href="${pageContext.request.contextPath}/spot.do?pageNow=${page.totalPageCount}">尾页</a>  
            </c:otherwise>  
        </c:choose>  
    </div>  
    <!-- 分页功能 End -->  
