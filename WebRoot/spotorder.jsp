...
<div id="hidebg"></div><!--遮罩层-->
  ...
  <div class="contentBox">
    <form action="${pageContext.request.contextPath}/ordersubmit.do?orderName=<%=orderName %>&theme=<%=theme %>&address=<%=address %>&type=<%=type %>&price=<%=price %>&wayofpayment=<%=wayofpayment %>"
			method="post">
      <ul>
        <li>景点名称:<span><%=orderName %></span></li>
        <li>特色:<span><%=theme %></span></li>
        <li>位置:<span><%=address %></span></li>
        <li>门票类型:<span><%=type %></span></li>
        <li>门票价格:<span class="singlePrice"><%=price %></span></li>
